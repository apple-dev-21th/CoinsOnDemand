...
<style>
    ...
    <?php if ($coin_type == 'eagle') {?>
        ...
        .coincorner {
            height: 353px !important;
            width: 353px !important;
            background-size: 353px 353px !important;
            top: 3px !important;
            left: 0px !important;
        }
...
<?php
@$template_img = $this->session->userdata('design_templatimg');
echo $template_img;
?>
...
<div class="container white">
    ...
    <div class="leftcont">
        <div class="glossymenu">
            <a  id="step2" class="menuitem submenuheader font_color" style="display:none;" ></a> 
            <a  id="step2" class="menuitem submenuheader font_color" >Step 1: Upload your photo </a> 
            ...
            <div class="submenu second1 open" contentindex="1c" style="display: none;">
                <select  name="category" class="theme" id="category" onchange="gettemplate(this.value)" style="border: 1px solid #3E3E3C; color: #3E3E3C;">
                    <option>Choose a Design from Categories</option>
    ...
    <div class="rightcont">
        <div class="wrapper fullsizeofimagesave" style="position:relative;">
            ...
            <div class="row" style="position: absolute; z-index:1000; top:0px; left:0px; width:203px !important;">
                <div class="col-md-12 pull-left "> <a class="startover" href="javascript:;" onclick="restfunction()"></a>
                </div>
...
?>
        ...
        $("#floatCurveText").css('margin-top', '-6px');
        $("#floatCurveTextBottom").css('margin-top', '-4px');
...
//                    logging:true,
            ...
            onrendered: function(canvas) {
                ...
                var Parameters = "image=" + output + '&template=' + '<?php echo $this->session->userdata('templatimg'); ?>' + '&coin_type=' + coin_type;
    ...
    function gettemplate(id) {
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo base_url(); ?>select_template/getdesigntemplatename/',
            data: {categoryid: id},
            success: function(data) {
    ...
    function changetemplate(img) {
        $.ajax({
            ...
            success: function(data) {
                ...
                setCookie('design_template',data,1)
                stemplate = 1;
            }
        });
    }
...
if( toptextstyle !== ''){
 ...
 $('#bottomCurvedText').remove();
            ...
            if( bottomtextstyle !== '') {
            var res = bottomtextstyle.split(";");   // Update saved font family
             for (i = 0; i < res.length; i++) {
                    if (res[i].match(/font-family:*/)) {
                        var fontfamily = res[i];
                        var res1 = fontfamily.split(":");
...
$(window).load(function() {
 var design_template = getCookie('design_template');
 if(design_template != ''){
     var link = '<?php echo base_url(); ?>' + 'assets/uploads/' + design_template;
                $('.coincorner').css("display", "block");
                $('.coincorner').css("background", "url(" + link + ")");
                $('.coincorner').css("height", "350px");
                $('.coincorner').css("width", "350px");
